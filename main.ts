...
const palavrasCoringa: string[] = ['COISA', 'NADA', 'XABLAU', 'BLAH'];

function mostrarDificuldades(): void {
  const opcoes = document.getElementById("opcoes-dificuldade");
  opcoes?.classList.toggle("hidden");
}

function selecionarDificuldade(nivel: string): void {
  dificuldadeSelecionada = nivel;
  const texto = document.getElementById("dificuldade-escolhida");
  const botaoJogar = document.getElementById("botao-jogar") as HTMLButtonElement;
  if (texto) texto.innerText = `Dificuldade: ${nivel.toUpperCase()}`;
  if (botaoJogar) botaoJogar.disabled = false;
}
...
function iniciarJogo(): void {
  ...
  document.querySelector('.tela-jogo')?.classList.remove('hidden');
  const nivelJogo = document.getElementById("nivel-jogo");
  if (nivelJogo) nivelJogo.innerText = dificuldadeSelecionada.toUpperCase();
...
function voltarInicio(): void {
  ...
  document.querySelector('.tela-inicial')?.classList.remove('hidden');
  document.querySelector('.tela-jogo')?.classList.add('hidden');
  dificuldadeSelecionada = null;
  const texto = document.getElementById("dificuldade-escolhida");
  const botaoJogar = document.getElementById("botao-jogar") as HTMLButtonElement;
  if (texto) texto.innerText = '';
  if (botaoJogar) botaoJogar.disabled = true;
...
function gerarTabuleiroComPalavras(): void {
  let qtd = 5;
  if (dificuldadeSelecionada === 'médio') qtd = 10;
  if (dificuldadeSelecionada === 'difícil') qtd = 15;
...
function posicionarPalavra(palavra: string): void {
...
function preencherLetrasAleatorias(): void {
...
function renderizarTabuleiro(): void {
...
function ativarSelecaoPorArrasto(): void {
  let selecionando = false;
  let selecionadas: HTMLElement[] = [];
  let direcao: "horizontal" | "vertical" | "diagonal" | null = null;
  const tabuleiroEl = document.getElementById("tabuleiro")!;
  function calcularDirecao(ultima: HTMLElement, atual: HTMLElement): "horizontal" | "vertical" | "diagonal" | null {
    ...
    if (Math.abs(x1 - x2) === Math.abs(y1 - y2)) return "diagonal";
    return null;
  }
  function podeSelecionar(ultima: HTMLElement, atual: HTMLElement): boolean {
    if (selecionadas.includes(atual)) return false;
  ...
  tabuleiroEl.onmousedown = (e) => {
    if ((e.target as HTMLElement).classList.contains("cell")) {
      ...
      const celula = e.target as HTMLElement;
  ...
  tabuleiroEl.onmousemove = (e) => {
    ...
    if (!alvo.classList.contains("cell")) return;
...
function marcarPalavraEncontrada(celulas: HTMLElement[]) {
  celulas.forEach(cel => {
...
function atualizarLista(palavra: string) {
